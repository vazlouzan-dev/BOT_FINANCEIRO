...
with tab_dashboard:
    ...
    if all_assets_data:
        ...
        SOURCE_BADGE = {
            "finnhub":  "<span style='background:#00c80518; color:#00c805; border-radius:3px; "
                        "padding:1px 5px; font-size:0.7rem;'>● live</span>",
            "yfinance": "<span style='background:#88888818; color:#888; border-radius:3px; "
                        "padding:1px 5px; font-size:0.7rem;'>~ 15m</span>",
        }
        ...
        for group_key in GROUP_ORDER:
            ...
            for ak in group_keys:
                ...
                src    = info.get("price_source", "yfinance")
                b_icon = BIAS_ICONS.get(b, "⚪")
                price_fmt = (
                    f"{close:.4f}" if close and close < 100
                    else f"{close:.2f}" if close else "N/D"
                )
                rows.append({
                    "Ativo":     name,
                    ...
                    "Conf.":     fmt_conf(c),
                    "Preço":     price_fmt,
                    "Fonte":     "live" if src == "finnhub" else "~15m",
                })
